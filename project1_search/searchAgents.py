    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append(((nextPos, visitedCorners), action, cost))
...
def cornersHeuristic(state, problem):
    ...
    """
    cornersLeft = list(problem.corners)
    cornersVisited = list(state[1])
    cornersLeft = [n for n in cornersLeft if n not in cornersVisited]
    curPos = state[0]
    h = 0
    ...
    while len(cornersLeft) != 0:
        ...
        h += min(distances)
        curPos = cornersLeft[distances.index(min(distances))]
        cornersLeft.remove(curPos)
