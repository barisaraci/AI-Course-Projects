...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        beliefList = util.Counter()

        if noisyDistance == None:
            beliefList[self.getJailPosition()] = 1.0
        else:
            for pos in self.legalPositions:
                trueDistance = util.manhattanDistance(pos, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    beliefList[pos] = emissionModel[trueDistance] * self.beliefs[pos]

        beliefList.normalize()
        self.beliefs = beliefList
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            ...
            for newPos, prob in dist.items():
                beliefList[newPos] += self.beliefs[pos] * prob
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for pos in self.legalPositions:
                ...
                beliefList[pos] += emissionModel[dist] * beliefDist[pos]

            if beliefList.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                for i in range(self.numParticles):
                    self.particles[i] = util.sample(beliefList)
